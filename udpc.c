...
int main(int argc,char** argv)
{
	...
	if (0>(len=sendto(sockfd,send_buf,strlen(send_buf),0,(struct sockaddr*)&server,sizeof(server))))
	{
	...
	int lent = sizeof(server);
	if (0>(len=recvfrom(sockfd,recv_buf,2048,0,(struct sockaddr*)&server,&lent)))
	{
